...
export const generateText = async (req, res) => {
  try {
    ...
    if (chat.messages.length === 1 || (chat.messages.length === 2 && chat.title === 'New Chat')) {
...
export const generateImage = async (req, res) => {
  try {
    ...
    const response = await fetch('https://api.imagekit.io/v1/files/ai/text-to-image', {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      return res.json({ success: false, message: data.message || 'Image generation failed' });
    }

    const generatedImageUrl = data.imageUrl || data.url || (data.data && data.data.imageUrl);
